...
function ifma_widgets_init() {
	...
	// Area 2, located below the Nav Area in the sidebar. Empty by default.
	register_sidebar( array(
...
if ( ! function_exists('ifma_login_logo') ) {
	function ifma_login_logo() { ?>
	    <style type="text/css">
	...
	<?php }
	add_action( 'login_enqueue_scripts', 'ifma_login_logo' );

	function ifma_login_logo_url() {
		return home_url();
	}
	add_filter( 'login_headerurl', 'ifma_login_logo_url' );

	function ifma_login_logo_url_title() {
 ...
 * @return string
 */
function ifma_login_redirect( $redirect_to, $request, $user ) {
