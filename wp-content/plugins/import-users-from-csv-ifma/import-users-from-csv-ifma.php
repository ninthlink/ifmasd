...
/* All IFMA customized lines can be found by [CUSTOM for IFMA] */
...
<?php
	...
	public static function import_csv( $filename, $args ) {
		...
		while ( ( $line = $csv_reader->get_row() ) !== NULL ) {
			...
			/* [CUSTOM for IFMA] Set all imported users as subscribers, and define other access rules */
			...
			if ( ! $update && empty( $userdata['user_pass'] ) )
				$userdata['user_pass'] = ucwords(substr($userdata['first_name'], 0, 1)) . ucfirst(strtolower(preg_replace('/[^A-Za-z0-9\-]/', '', $userdata['last_name']))); /* [CUSTOM for IFMA] PW is First letter of first name (caps) and full last name (first letter caps) i.e DSutter */
				//$userdata['user_pass'] = wp_generate_password( 12, false ); <-- this was the original plugin method - randomly generated
			if ( $update )
